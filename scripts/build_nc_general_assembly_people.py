#!/usr/bin/env python3
"""
Seed person + membership records for current North Carolina General
Assembly members (House + Senate), from openstates/people's NC legislature
bulk data -- the same source build_nc_general_assembly.py used to build the
district jurisdiction/organization/post structure this attaches to.
...
only lives on candidacies[], which a future NC SBOE enrichment phase would
populate (see build_ocpf_enrichment.py's MA equivalent).

Idempotent by id AND by filename -- never overwrites.

Usage: python3 build_nc_general_assembly_people.py [--write]
...
import build_nc_general_assembly as ga  # reuse slugify + CHAMBER_INFO

REPO = Path(__file__).resolve().parent.parent
DATA_DIR = REPO / "data" / "us" / "nc"
LEGISLATURE_CACHE = Path("/tmp/nc_openstates_legislature")

HEADER = (
    "# Machine-generated from openstates/people's NC legislature bulk data\n"
    "# by scripts/build_nc_general_assembly_people.py.\n"
)
...
def slugify(name):
    return ga.slugify(name)


def load_org_post(chamber, slug):
    suffix = ga.CHAMBER_INFO[chamber]["file_suffix"]
...
def build_person_and_membership(doc, people_by_name):
    ...
    if office and office.get("address"):
        addr = {"classification": "capitol", "name": "Legislative Building Office", "address": office["address"]}
        if office.get("voice"):
    ...
    if not sources:
        sources = [{"url": "https://www.ncleg.gov/"}]
