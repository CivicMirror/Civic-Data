...
| `irrigation_district` | Irrigation District | Water Code Ch. 58 | Elected | `sewer` | **Done (structure)** — 17 of 18 raw rows seeded (jurisdiction + board + 5 director posts each, board fixed at 5 per Sec. 58.071, no override clause). Cleanest batch yet: 0 name-vs-Entity-Type mismatches; only 1 exact-duplicate name (two ACTIVE filings from different years) needed dedup. No officeholders yet. |
| `groundwater_conservation_district` | Groundwater Conservation District (GCD) | Water Code Ch. 36 | Individually confirmed per district (not assumed) | `sewer` | **Done (structure)** — 55 districts seeded total, each with an individually-confirmed seat count (5 to 12, no uniform default like WCID/FWSD/WID/Irrigation): 39 from the "Groundwater Conservation District" Entity Type bucket, plus 16 more folded in from the separate "Underground Water Conservation District" bucket after research confirmed that Entity Type distinction carries no statutory meaning (`scripts/seed_tx_gcd_from_uwcd_bucket.py`). 36 total confirmed appointed across both buckets, excluded entirely (full list with appointing authorities in each script's docstring). 1 more (Permian Basin UWCD) excluded on a primary-source conflict — its own current website says appointed, an older bill analysis says elected; flagged for a follow-up statute check, not resolved. 0 undetermined. No officeholders yet. |
| `drainage_district` | Drainage District | Water Code Ch. 56 | Individually confirmed per district (not assumed) | `sewer` | **Done (structure, curated)** — 16 of 27 raw rows seeded, each with an individually-confirmed seat count (3, 5, or 7 — not the Sec. 56.052 default of 3 assumed blindly). 3 confirmed appointed (Cameron County DD#1, DeWitt/"Green DeWitt" DD, Jefferson County DD#6 — appointed by county commissioners court, excluded). 4 undetermined after a reasonable search, held back (Centex DD, Jackson County County Wide DD, Matagorda County DD#2, Webb County DD#1 — elected but seat count unconfirmed). 1 excluded entirely: Hidalgo County DD#1's "board" is literally the county commissioners court, no separate elected office exists. 2 rows reclassified to `conservation_and_reclamation_district:` (a distinct statutory category), 1 row reclassified to `water_control_improvement_district:`. See `scripts/seed_tx_drainage_jurisdictions_orgs_posts.py`. |
| `conservation_and_reclamation_district` | Conservation and Reclamation District | Ch. 62, Acts of the 52nd Legislature, 1951, or an individual special act (e.g. HB 447, 56th Leg., 1959) / Article XVI Sec. 59, Texas Constitution (varies by district — a distinct lineage from Water Code Ch. 56/49) | Elected (individually confirmed) | `sewer` | **Done (structure)** — 3 districts: Brazoria County CRD#3 and Matagorda County CRD#1 (both misfiled under Entity Type "Drainage District", discovered during the Drainage District research pass, 3 elected Commissioner seats each), plus Real-Edwards CRD (misfiled under "Underground Water Conservation District", discovered during the GCD follow-up research, 9 elected seats: 4 Edwards County + 4 Real County + 1 at-large — its enabling act citation differs from the other two, not yet confirmed whether that's the same umbrella chapter or a separate one). `scripts/seed_tx_conservation_reclamation_district.py`. |
| `edwards_aquifer_authority` | Edwards Aquifer Authority (EAA) | Its own unique enabling act (the Edwards Aquifer Authority Act) | Elected (15 of 17 board seats; 2 appointed non-voting, out of scope) | `sewer` | **Done (structure)** — 1 entity, misfiled under Entity Type "Underground Water Conservation District" and listed under Bexar County alone in the raw sheet, though its board spans single-member districts across Bexar, Comal, Guadalupe, Hays, Caldwell, Medina, Atascosa, and Uvalde counties. 15 elected single-member-district seats seeded; the board's 2 appointed non-voting seats excluded. `scripts/seed_tx_edwards_aquifer_authority.py`. |
