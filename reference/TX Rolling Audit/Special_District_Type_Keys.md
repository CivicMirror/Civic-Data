...
| `groundwater_conservation_district` | Groundwater Conservation District (GCD) | Water Code Ch. 36 | Individually confirmed per district (not assumed) | `sewer` | **Done (structure)** — 39 of 59 raw rows seeded, each with an individually-confirmed seat count (5 to 12, no uniform default like WCID/FWSD/WID/Irrigation). 20 confirmed appointed by one or more counties'/cities' governing bodies, excluded entirely — see `scripts/seed_tx_gcd_jurisdictions_orgs_posts.py` for the full list and appointing authorities. 0 undetermined. No officeholders yet. |
...
| `conservation_and_reclamation_district` | Conservation and Reclamation District | Ch. 62, Acts of the 52nd Legislature, 1951 / Article XVI Sec. 59, Texas Constitution (varies by district — a distinct lineage from Water Code Ch. 56/49) | Elected (individually confirmed) | `sewer` | **Done (structure)** — 2 districts (Brazoria County CRD#3, Matagorda County CRD#1), both misfiled under Entity Type "Drainage District" in the Comptroller's SPDPID, discovered during the Drainage District research pass. 3 elected Commissioner seats each. `scripts/seed_tx_conservation_reclamation_district.py`. |
